...
namespace vplace_impl
{
    ...
    { 
        ...
        Adjustable(unsigned Min=MIN, unsigned Max=MAX)  : minmax(Min, Max){} 
    ...
    { 
        Fixed(unsigned weight, unsigned min_=MIN,unsigned max_=MAX): Adjustable(min_,max_), weight_(weight){}
    ...
    { 
        Percent(double percent_,unsigned min_,unsigned max_):Fixed (static_cast<unsigned>(fx*percent_),min_,max_){}
...
template <class Adj= Adjustable , class Fld=Widget> struct Field : Fld, Adj
{
    ...
    { 
        ...
        virtual        ~Widget    (){API::umake_event(destroy_evh);}
    ...
    struct Splitter: public Field<Fixed,Widget>
    {
        ...
        Splitter(window pw, double  init_perc=0.3):Field<Fixed,Widget>(nullptr,unsigned(4)),init_perc(init_perc) 
        {
            splitter_.create(pw);
    ...
    Splitter * div_h::create_splitter()
        {
            ...
            splitter->leaf_left_.reset(dynamic_cast<Field<Percent,division>*>(new Field<Percent,div_h> (splitter->init_perc)));//percent_div_h
    ...
    {   fastened_in_div.clear ();
        ...
        {                             /// find in the place global list of fields all the fields attached to it
            ...
            for (auto fi=r.first ; fi != r.second ; ++fi)      
            {
                ...
                auto div=dynamic_cast<division*>(fi->second.get());
                if (div) div->populate_children (place_impl_);
                if (gap) 
    ...
    void implement::collocate()
	{
		if(root_division && parent_window_handle)
		{
            ...
            {      
               ...
               recollocate = false;
               root_division->collocate_field(r/*=API::window_size(this->parent_window_handle)*/);
            }
            //			                                //rectangle r; // debugg
	...
	private:
	 ...
	 private:
		...
		{
			fld->MinMax (*this);
            place_impl_->fields.emplace(name,std::unique_ptr<Adjustable>( fld));
   ...
   //                 }
	...
	{
		return new vplace_impl::Field<vplace_impl::Percent, vplace_impl::Widget>(wd, per, w.min ,w.max );//percent_widget
	}
...
//end namespace nana
    ...
    //    adj   pre_place(unsigned t_w,                        adj& fixed = adj() )      
    //                {  
    //                    ++fixed.count_adj;     
    //                    fixed.min += min;    
    //                    return  fixed;        
    //                }
    //    adj   end_place(unsigned t_w,const adj& fixed = adj(), adj& adj_min = adj() )      
    //    {   
    //        if ( t_w      <   fixed.weigth + fixed.min         )  
    //                                                        { adj_min.weigth += min; return adj_min; }
    //        if ( t_w <    min * fixed.count_adj + fixed.weigth )
    ...
    //        ++adj_min.count_adj;   return  adj_min;        
    //    }
    //    unsigned weigth(unsigned t_w,const adj& fixed,const adj& adj_min ) 
    //    {   
    //        if ( t_w      <   fixed.weigth + fixed.min         )   
    //                                                                    {return min; }
    //        if ( t_w <    min * fixed.count_adj   + fixed.weigth   )   
    ...
    //using adj_gap   = Field<Adjustable,Gap >;     //  IAdjustable<Gap_field> ;
    //using adj_widget= Field<>;              //  IAdjustable<Widget_field>;
    //using adj_room  = Field<Adjustable,Room >;    //  IAdjustable<Room_field> ;
    //using adj_div_h = Field<Adjustable,div_h>;         //  IAdjustable<div_h> ;
    //using adj_div_v = Field<Adjustable,div_v>;         //  IAdjustable<div_v> ;
    //using adj_div_grid = Field<Adjustable,div_grid>;   //  IAdjustable<div_grid> ;
    ...
    //using fixed_gap    = Field<Fixed,Gap>;    //  IFixed<Gap_field> ;
    //using fixed_widget = Field<Fixed,Widget>; //   IFixed<Widget_field> ;
    //using fixed_room   = Field<Fixed,Room>;   //  IFixed<Room_field> ;
    //using fixed_div_h  = Field<Fixed,div_h>;        //  IFixed<div_h> ;
    //using fixed_div_v  = Field<Fixed,div_v>;        //  IFixed<div_v> ;
    //using fixed_div_grid = Field<Fixed,div_grid>;   //  IFixed<div_grid> ;
